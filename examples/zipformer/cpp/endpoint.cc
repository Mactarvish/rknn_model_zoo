...
static auto logger = GetLogger("endpoint.cc");
...
static bool RuleActivated(const EndpointRule &rule,
  ...
  bool ans = (contain_nonsilence || !rule.must_contain_nonsilence) && // 说过话（可选） && 语音足够长 && 静音足够长
...
bool Endpoint::IsEndpoint(const int num_frames_decoded,
  ...
  if (RuleActivated(config_.rule1, "rule1", trailing_silence,
      ...
      RuleActivated(config_.rule3, "rule3", trailing_silence,
                    utterance_length)) 
  {
    logger->info("Endpoint detected: num_frames_decoded={}, trailing_silence_frames={}, frame_shift_in_seconds={}",
                 num_frames_decoded, trailing_silence_frames, frame_shift_in_seconds);
    return true;
  }
  return false;
}
